...
# fatal error if L<parse_date> tries to construct a L<DateTime> object with
...
L<strftime|DateTime/strftime> method.  L<Rose::DateTime::Util> calls
L<DateTime>'s L<strftime|DateTime/strftime> method when interpolating these
formats.

Note that the C<%t> and C<%F> formats are I<not> passed to
L<strftime|DateTime/strftime>, but are handled by L<Rose::DateTime::Util>
instead.  See the "Non-standard formats" section below.

The L<strftime|DateTime/strftime>-compatible formats listed below have been
transcribed from the L<DateTime> documentation for the sake of convenience,
but the L<DateTime> documentation is the definitive source.

Using any format strings not in the L<strftime|DateTime/strftime>-compatible
set will be slightly slower.

B<strftime-compatible formats>
...
where "method" is a valid L<DateTime.pm> object method.
...
L<Rose::DateTime::Util-E<gt>error|error>.
...
L<set_time_zone(TIMEZONE)|DateTime/set_time_zone> called on it if there is a
TIMEZONE argument.

Since the time zone is not part of any of the supported date string formats,
L<parse_date> takes an optional TIMEZONE argument which is passed to the
L<DateTime> constructor as the value of the L<time_zone|DateTime/time_zone>
parameter.  In the absence of a TIMEZONE argument to L<parse_date>, the time
zone defaults to the value returned by C<Rose::DateTime::Util>'s L<time_zone>
class method ("floating", by default)

The formats understood and their interpretations are listed below.  Square
brackets are used to indicate optional portions of the formats.
...
C<DateTime-E<gt>new(...)> as the value of the L<time_zone|DateTime/time_zone>
parameter when L<parse_date> creates the C<DateTime> object that it returns. 
The default value is "floating".
